 ...
 * @param	address		the subpage view button address
 * @param	popupName	the name of the subpage to show
 * @param	position	the position of the subpage in sub-page view(from 0)
 */
define_function showSubPageEx(dev tp, 
					integer address, 
					char subPageName[], 
					integer position
					) 
{
	send_command tp, "'^SSH-', itoa(address), ',', subPageName, ',', itoa(position)";
}
